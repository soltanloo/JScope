...
export class Coverage {
    ...
    private _relativePaths: boolean = false;
    ...
    setProjectInfo(projectPath: string, projectName: string, relativePaths: boolean = false) {
        ...
        return _logs.map((log) => {
            if (log.location) {
                ...
                if (this._relativePaths) {
                    log.location = log.location.substring(relativePathStartInd + 1)
                } else {
